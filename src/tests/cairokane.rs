...
use cairokane_support::{address::Payload, constants::SHORCELL_FACTORY_ID};
...
use crate::tests::std::cairokane_contract_build;
#[allow(unused_imports)]
...
fn burn(proof: Vec<u8>) -> Transaction {
  let protostone: Protostone = Protostone {
    burn: None,
    edicts: vec![],
    pointer: Some(0),
  ...
  // verify proof
  tx.input[0].witness = RawEnvelope::from(proof.to_vec()).to_gzipped_witness();
  tx
}

#[wasm_bindgen_test]
fn test_cairokane() -> Result<()> {
    clear();
    ...
    let mut test_block = alkane_helpers::init_with_multiple_cellpacks_with_tx(
        [cairokane_contract_build::get_bytes()].into(),
        cellpacks,
    );
